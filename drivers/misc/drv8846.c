...
static int drv8846_gpio_config(struct drv8846_soc_ctrl *mctrl)
{
	...
	rc = gpio_request_one(mctrl->gpio_mode0, GPIOF_OUT_INIT_HIGH,
			      "motor-mode0");
	if (rc < 0) {
	...
	rc = gpio_request_one(mctrl->gpio_mode1, GPIOF_OUT_INIT_HIGH,
			      "motor-mode1");
	if (rc < 0) {
	...
	rc = gpio_request_one(mctrl->gpio_dir, GPIOF_OUT_INIT_HIGH,
			      "motor-dir");
	if (rc < 0) {
	...
	rc = gpio_request_one(mctrl->gpio_sleep, GPIOF_OUT_INIT_HIGH,
			      "motor-sleep");
	if (rc < 0) {
	...
	rc = gpio_request_one(mctrl->gpio_pwren, GPIOF_OUT_INIT_HIGH,
			      "motor-pwr");
	if (rc < 0) {
