...
*/
static struct seq_operations fts_seq_ops = { .start = fts_seq_start,
					     .next = fts_seq_next,
					     .stop = fts_seq_stop,
					     .show = fts_seq_show };
	...
	if (numberParam >= 1) {
		...
		case CMD_WRITEREAD_BYTE:
			if (numberParam >= 5) {
				...
				readData = (u8 *)kmalloc(
					(byteToRead + mess.dummy) * sizeof(u8),
					GFP_KERNEL);
				res = fts_writeRead_dma_safe(
					&cmd[1], temp, readData,
					byteToRead + mess.dummy);
				size += (byteToRead * sizeof(u8));
		...
		case CMD_WRITETHENWRITEREAD_BYTE:
			if (numberParam >= 6) {
				...
				readData = (u8 *)kmalloc(
					byteToRead * sizeof(u8), GFP_KERNEL);
				res = fts_writeThenWriteRead(
					&cmd[3], cmd[1], &cmd[3 + (int)cmd[1]],
					cmd[2], readData, byteToRead);
				size += (byteToRead * sizeof(u8));
			...
			if (numberParam >= 6) {
				if (cmd[2] <= sizeof(u64)) {
					...
					u8ToU16_be(&cmd[numberParam - 3],
						   &byteToRead);
					readData = (u8 *)kmalloc(
						byteToRead * sizeof(u8),
						GFP_KERNEL);
					logError(
						0,
						"%s addr = %016X byteToRead = %d \n",
						tag, addr, byteToRead);
					res = fts_writeReadU8UX(
						cmd[1], cmd[2], addr, readData,
						byteToRead,
						cmd[numberParam - 1]);
					size += (byteToRead * sizeof(u8));
		...
		case CMD_WRITEU8UXTHENWRITEU8UX_BYTE:
			if (numberParam >= 6) {
				if ((cmd[2] + cmd[4]) <= sizeof(u64)) {
					...
					logError(0, "%s addr = %016X %ld \n",
						 tag, addr, addr);
					res = fts_writeU8UXthenWriteU8UX(
						cmd[1], cmd[2], cmd[3], cmd[4],
						addr, &cmd[5 + cmd[2] + cmd[4]],
						(numberParam - cmd[2] - cmd[4] -
						 5));
				} else {
					logError(1, "%s Wrong address size! \n",
						 tag);
		...
		case CMD_READTOTMSCOMPDATA:
			if (numberParam == 2) {
				logError(0,
					 ...
					 tag);
				res = readTotMutualSenseCompensationData(
					cmd[1], &totCompData);
