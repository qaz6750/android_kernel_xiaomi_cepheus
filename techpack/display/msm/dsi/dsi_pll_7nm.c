...
static inline bool dsi_pll_7nm_is_hw_revision_v1(
		struct dsi_pll_resource *rsc)
{
	return (rsc->pll_revision == DSI_PLL_7NM) ? true : false;
}

static inline bool dsi_pll_7nm_is_hw_revision_v2(
		struct dsi_pll_resource *rsc)
{
	return (rsc->pll_revision == DSI_PLL_7NM_V2) ? true : false;
}
...
static int dsi_pll_7nm_get_gdsc_status(struct dsi_pll_resource *rsc)
{
	u32 reg = 0;
	bool status;
				  ...
				  struct dsi_pll_resource *rsc)
{
	...
	if (dsi_pll_7nm_is_hw_revision_v1(rsc))
		DSI_PLL_REG_W(pll_base, PLL_GEAR_BAND_SELECT_CONTROLS, 0x21);
	...
	case DSI_PLL_7NM:
		...
		break;
	case DSI_PLL_7NM_V2:
		DSI_PLL_REG_W(pll_base, PLL_IFILT, 0x22);
		break;
	case DSI_PLL_7NM_V4_1:
	default:
...
static void dsi_pll_init_val(struct dsi_pll_resource *rsc)
{
	...
	if (dsi_pll_7nm_is_hw_revision_v1(rsc))
		DSI_PLL_REG_W(pll_base, PLL_BIAS_CONTROL_1, 0x00000066);
	else
...
static int dsi_pll_7nm_lock_status(struct dsi_pll_resource *pll)
{
	int rc;
	u32 status;
...
static int vco_7nm_prepare(struct clk_hw *hw)
{
	...
	if ((pll->vco_cached_rate != 0) &&
		...
		rc = vco_7nm_set_rate(hw, pll->vco_cached_rate,
				pll->vco_cached_rate);
		if (rc) {
						...
						unsigned long parent_rate)
{
	...
	*/
	...
	if (!dsi_pll_7nm_get_gdsc_status(pll)) {
...
static struct clk_hw *dsi_pllcc_7nm[] = {
				  ...
				  struct dsi_pll_resource *pll_res)
{
	...
	int num_clks = ARRAY_SIZE(dsi_pllcc_7nm);
	struct regmap *rmap;
